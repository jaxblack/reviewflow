...
| 处理效率 | 智能优先级、队列聚焦、状态/风险快捷筛选、标题/作者搜索、四种排序、⌘K 聚焦和方向键切换 |
...
- **应用级导航**：角色可见队列、系统文档、权限管理和当前操作人固定在左侧，不与业务内容争夺空间。
- **唯一请求清单**：工作台、待我审核、我的提交、已参与和管理员全量只是过滤视角；每条内容在当前清单只渲染一次。
- **运营优先级**：默认把高风险待办、普通待办、其他审核中、已拒绝和草稿依次前置，也可切换为更新时间或标题排序。
- **高效定位**：支持组合筛选、快捷条件、`⌘K` 聚焦搜索，以及方向键在结果间切换。
- **审计详情**：粘性操作栏、请求编号、内容与进度并排展示、完整轮次时间线、审计 JSON 导出和明确的成功/失败反馈。
...
工作台通过固定导航展示当前身份可见的语义队列及数量，中间唯一请求清单支持智能优先级、四种排序、标题/作者搜索、状态/风险组合筛选和独立滚动；右侧持续展示请求生命周期、当前轮次、不可变历史与审计导出。重复执行种子不会创建重复数据，场景和建议演示顺序见[演示场景](docs/demo-scenarios.md)。
