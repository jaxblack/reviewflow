...
清单默认按业务优先级排序：高风险待办、普通待办、其他审核中、已拒绝、草稿、已通过；用户也可切换最近更新、最早更新和标题排序。状态/风险组合筛选、快捷筛选、始终可见的重置按钮、`⌘K` 搜索、方向键切换、请求 ID 复制、审计 JSON 导出和成功通知均是前端效率能力，不替代任何服务端权限或状态校验。

侧栏额外提供审核中、高风险和已拒绝状态视图，并提供六步新手引导。“创建草稿”和“提交审核”是两个独立步骤，以明确 DRAFT → IN_REVIEW 的状态边界；其余步骤会切换到 Alice、Bob、Chen、Diana 对应身份并定位示例请求。当前操作人区域显示中文角色，以及“预置账号”或“自定义账号”来源。`Eva Ramp-up` 等名称是公开验收中创建的自定义用户，不是系统内置账号。
