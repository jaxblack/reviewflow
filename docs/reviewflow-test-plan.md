...
1. 左侧打开“五步完成核心验收”，每一步展示操作人、操作步骤和明确预期结果。
2. 分别点击 Alice、Bob、Chen、Diana 步骤，服务端会话切换到目标身份并定位对应示例数据。
3. 当前操作人区域展示中文角色和“预置账号”；管理员创建的用户展示“自定义账号”。
4. `Eva Ramp-up` 等引导创建用户必须标为自定义，不能伪装为预置账号。
5. Alice 的“待我审核”为空时明确解释自审禁令；Diana 不显示个人待审队列，指标改为全局待审核数量。
