...
1. 使用常用 PC 视口切换 Diana，页面右上角出现“用户与角色”入口。
2. 管理中心展示四个预置用户的角色、内容数和审核次数。
...
本轮产品验收只覆盖 PC 主从工作台和 PC 管理弹窗；移动视口属于后续适配，不作为当前退出条件。
...
- Playwright 在常用 PC 视口检查主从工作台和管理弹窗无文本溢出、遮挡或不可操作控件；移动视口后续单独验收。
