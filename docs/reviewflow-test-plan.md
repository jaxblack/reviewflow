...
1. 新建或更新请求后，默认最近更新排序将其展示在当前队列首位。
2. 聚焦“待我审核”并选择“高风险”快捷筛选，列表只保留匹配请求且同一内容不重复。
3. 切换智能优先级、最早更新和标题排序，当前选中请求保持稳定。
4. 使用 `⌘K` 或 `/` 聚焦搜索，方向键切换当前结果并同步刷新右侧详情。
5. 导出审计记录，下载文件名包含请求 ID，JSON 只包含当前详情 API 已授权返回的数据。
6. 创建、提交、审核和身份切换后出现明确反馈；失败响应不得显示成功通知。
7. 组合搜索、状态和风险条件后点击“重置筛选”，三类条件一次清空，当前角色队列保持不变。

### 14.6 E2E-006：新手引导与账号来源

1. 左侧打开“六步完成核心验收”，每一步展示操作人、操作步骤和明确预期结果。
2. “创建草稿”和“提交审核”必须是两个独立步骤；前者停留 DRAFT 且无 round，后者进入 IN_REVIEW 并创建 R1。
3. 引导不填写或搜索固定标题；分别点击 Alice、Bob、Chen、Diana 步骤后，按最近更新排序定位同一条刚创建或刚处理的请求。
4. 当前操作人区域展示中文角色和“预置账号”；管理员创建的用户展示“自定义账号”。
5. `Eva Ramp-up` 等引导创建用户必须标为自定义，不能伪装为预置账号。
6. Alice 的“待我审核”为空时明确解释自审禁令；Diana 不显示个人待审队列，指标改为全局待审核数量。
7. 空标题和正文显示业务示例占位词；多轮详情中的 R1、R2 默认全部展开；左下角角色切换使用可识别的高对比按钮。
8. “暂存草稿”只产生 DRAFT；“直接提交审核”依次保存并提交，成功后形成 R1，第二步失败时明确显示草稿已保存且刷新最新版本。
